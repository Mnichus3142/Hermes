<script lang="ts">
    ...
    let thirdSpanClicked = $derived(
        focusedField === "confirm" || hoveredField === "confirm",
    );

    let showPasswordStrength = $derived(
        passwordCheck.firstPassword.length > 0 &&
            !passwordCheck.conditions.allConditionsMet,
    );
    ...
    $effect(() => {
        passwordCheck.secondPassword = clearSpaces(
    ...
    async function handleInternalRegistration(e: Event) {
        ...
        if (
            ...
            passwordCheck.firstPassword !== passwordCheck.secondPassword
        ) {
            createNewNotification({
                title: "Could not register",
        ...
        try {
            ...
            if (error.message !== "409") {
                ...
                createNewNotification({
                    ...
                    message:
                        error.message ||
                        "There was an unknown error during registration",
                    type: "error",
                    duration: 3000,
                });
            }
...
>
    ...
    <form novalidate onclick={() => (focusedField = null)}>
        ...
        <h2 class="title" style:transform="translateY({titleY}px)">Register</h2>
