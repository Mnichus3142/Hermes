<script lang="ts">
    ...
    // Function to get what buttons should be shown
    const getButtons = async () => {
        fetch('/api/info/OAuth')
        .then(response => response.json())
        .then(data => {
            buttons = data;
        })
        .catch(error => {
            ...
            createNewNotification({
                title: 'Error during fetching buttons',
                message: error.message || 'There was an unknown error during fetching buttons',
                type: 'error',
                duration: 3000
            });
        });
